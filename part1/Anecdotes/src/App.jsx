...
const App = () => {
  ...
  const findHighestVote = (votes) => {
    const anecdoteNum = Object.keys(votes).reduce(
      (a, b) => votes[a] > votes[b] ? a : b
    )
    setHighestVote(anecdoteNum)
  }

  const handleRandomSelection = () => setSelected(Math.floor(Math.random() * anecdotes.length))
